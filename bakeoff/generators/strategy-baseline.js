...
// ═══════════════════════════════════════════════════════════════
{
  ...
  tbl(slide, riskRows, {
    x: 6.1, y: TOP + 2.81, h: 0.32,
    colW: [1.05, 0.55, 1.55, 0.60],
    fontSize: 9,
    hdrFill: C.amber,
  });
}
...
// ═══════════════════════════════════════════════════════════════
{
  ...
  slide.addText("⚠  Q3 Attrition: 6 departures (5.4% quarterly) — 3 engineering to FAANG. Comp bands adjusted +12% for senior engineers.", {
    x: 0.15, y: H - 0.55, w: 5.5, h: 0.22, fontSize: 7.5, color: C.amber, margin: 0
  });
}
...
// ─── WRITE FILE ───────────────────────────────────────────────
pres.writeFile({ fileName: "outputs/strategy-baseline.pptx" })
  .then(() => console.log("✓ outputs/strategy-baseline.pptx written"))
