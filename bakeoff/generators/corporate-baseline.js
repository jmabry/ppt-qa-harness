...
// ════════════════════════════════════════════════════════════
{
  ...
  sld.addText('A321XLR (4,700 nm range) unlocks routes impossible for 757 or A321neo: EWR to Bari, Split, Glasgow, Santiago de Compostela — point-to-point international with narrowbody economics.', {
    x: 0.4, y: 4.87, w: 9.2, h: 0.58, fontSize: 8.5, color: C.darkSlate, italic: true, valign: 'middle', wrap: true,
  });
}
...
// ════════════════════════════════════════════════════════════
{
  ...
  intData.forEach(([yr, val], i) => {
    ...
    if (bh > 0.22) {
      sld.addText(val, { x: bx, y: by + 0.04, w: barW, h: 0.2, fontSize: 8, bold: true, color: i === 3 ? C.white : C.navy, align: 'center' });
    }
    sld.addText(yr, { x: bx, y: 3.97, w: barW, h: 0.18, fontSize: 8, color: C.textMed, align: 'center' });
  });
  // Legend for the bar chart colors
  sld.addShape(prs.ShapeType.rect, { x: 0.25, y: 4.15, w: 0.18, h: 0.12, fill: { color: C.lightBlue } });
  sld.addText('2021–2023', { x: 0.46, y: 4.14, w: 1.1, h: 0.14, fontSize: 7, color: C.textLight });
  sld.addShape(prs.ShapeType.rect, { x: 1.65, y: 4.15, w: 0.18, h: 0.12, fill: { color: C.navy } });
  sld.addText('2024 (latest)', { x: 1.86, y: 4.14, w: 1.1, h: 0.14, fontSize: 7, color: C.textLight });
  ...
  styledTable(sld, safRows, 0.25, 4.57, 4.5, [1.0, 1.75, 1.75], 0.18, C.navyLight, C.white, C.white, C.textDark, 7.5);
...
// ─── Save ────────────────────────────────────────────────────────────────────
prs.writeFile({ fileName: 'outputs/corporate-baseline.pptx' })
  .then(() => console.log('Saved: outputs/corporate-baseline.pptx'))
